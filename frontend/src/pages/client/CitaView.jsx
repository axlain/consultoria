...
export function CitaView() {
  ...
  return (
    <ClientShell>
      ...
      {loading && (
        <div className="mt-20 text-center text-sm text-muted">Verificando cita…</div>
      )}

      {error && !loading && (
        <div className="px-5 mt-16 flex flex-col items-center gap-4 text-center">
          <span className="text-4xl">⚠️</span>
          <h2 className="text-xl font-bold text-ink">Enlace inválido</h2>
          <p className="text-sm text-muted">{error}</p>
          <Link
            to={`/demo/${tenant.slug}`}
            className="mt-2 rounded-2xl border border-line px-5 py-2.5 text-sm font-semibold text-ink no-underline hover:border-accent/40"
          >
            Ir al inicio
          </Link>
        </div>
      ...
      {cita && !loading && (
        <div className="motion-safe:animate-fade-in px-5 pt-10 pb-10">
          ...
          <div className="mb-5 flex items-center gap-3 pr-10">
            ...
            <div>
              <h1 className="text-xl font-bold text-ink leading-tight">Detalles de la cita</h1>
              <p className="text-sm text-muted">{cita.business_name || tenant.business?.name}</p>
            </div>
          </div>

          {/* Status badge */}
          <div className="mb-5">
            <span className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${STATUS_COLOR[cita.status] ?? 'border-line bg-surface text-muted'}`}>
          ...
          <dl className="divide-y divide-line rounded-2xl border border-line bg-surface overflow-hidden">
            {[
              ...
              <div key={label} className="flex items-baseline justify-between px-4 py-3">
                ...
                <dd className={`m-0 text-sm font-medium text-right ml-4 ${label === 'ID de cita' ? 'font-mono text-xs text-muted' : 'text-ink'}`}>{value}</dd>
              </div>
            ))}
          </dl>
          ...
          <Link
            to={`/demo/${tenant.slug}`}
            className="mt-7 inline-block w-full rounded-2xl border border-line px-5 py-3 text-center text-sm font-semibold text-ink no-underline transition-colors hover:border-accent/40"
          >
            ← Volver al inicio
          </Link>
        </div>
