  ...
  return (
    <ClientShell center className="flex flex-col items-center justify-center">
      ...
      <div className="motion-safe:animate-fade-in px-5 py-10 w-full">
        ...
        <h1 className="text-3xl font-extrabold text-ink mb-2">¡Cita confirmada!</h1>
        <p className="text-sm text-muted">Te esperamos en {tenant.business.name}.</p>
        ...
        {appointment && (
          <div className="mt-5 rounded-2xl border border-line bg-surface p-4 text-left">
            <dl className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-2 text-sm">
              ...
              <dd className="m-0 text-right font-medium text-ink">{appointment.date}</dd>
              <dt className="text-muted">Hora</dt>
              <dd className="m-0 text-right font-medium text-ink">{appointment.time}</dd>
              {appointment.id && (
                <>
                  <dt className="text-muted">Ref.</dt>
                  <dd className="m-0 text-right font-mono text-xs text-muted">#{appointment.id.slice(-8)}</dd>
                </>
              )}
            </dl>
          </div>
        ...
        {qrDataUrl && (
          <div className="mx-auto mt-6 flex w-fit flex-col items-center gap-3 rounded-2xl border border-line bg-surface p-5">
            <div className="flex items-center justify-center gap-2 text-accent mb-1">
              ...
              <span className="text-xs font-semibold text-muted">Código QR de tu cita</span>
            </div>
            ...
            <a
              ...
              className="rounded-2xl bg-accent px-5 py-2.5 text-sm font-bold text-[#0C0B09] no-underline transition-all hover:bg-accent-light active:scale-[0.98]"
            >
        ...
        <Link
          to={`/demo/${tenant.slug}`}
          className="mt-7 inline-block w-full rounded-2xl border border-line px-5 py-3 text-sm font-semibold text-ink no-underline text-center transition-colors hover:border-accent/40"
        >
          Volver al inicio
        </Link>
      </div>
