...
export function TenantHome() {
  ...
  return (
    <ClientShell reserveCta>
      ...
      {/* ── Stats strip ───────────────────────────────────────────── */}
      {google_rating && (
        <div className="grid grid-cols-3 border-y border-[#2A2520] mx-5">
          {[
      ...
      {/* ── Welcome banner ────────────────────────────────────────── */}
      {firstName && (
        <div className="mx-5 mt-5 flex flex-col items-center gap-3 rounded-2xl border border-[#C8973E]/20 bg-[#C8973E]/8 px-5 py-4 text-center sm:flex-row sm:justify-between sm:text-left">
      ...
      {/* ── Servicios ─────────────────────────────────────────────── */}
      <section aria-labelledby="services-heading" className="px-5 pt-8 pb-2">
        ...
        <div className="flex flex-col gap-3">
          {tenant.services.map((service) => (
            <Link
              key={service.id}
              to={`/demo/${tenant.slug}/reservar`}
              className="flex items-center gap-4 rounded-2xl border border-[#2A2520] bg-[#1E1B15] p-3 no-underline transition-all hover:border-[#C8973E]/50 active:scale-[0.99]"
            >
              <img
      ...
      {tenant.professionals && tenant.professionals.length > 0 && (
        <section className="pt-8 pb-2">
          <h2 className="text-xl font-bold text-[#F2EBE0] px-5 mb-5">Nuestro equipo</h2>
          <div className="flex gap-3 overflow-x-auto pb-2 -mx-0 px-5 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" style={{ scrollSnapType: "x mandatory" }}>
            {tenant.professionals.map((pro) => (
              <div
                key={pro.id}
                className="shrink-0 w-[140px] bg-[#1E1B15] border border-[#2A2520] rounded-2xl p-3 text-center"
      ...
      {tenant.reviews && tenant.reviews.length > 0 && (
        <section className="pt-8 pb-2">
          <h2 className="text-xl font-bold text-[#F2EBE0] px-5 mb-4">Lo que dicen</h2>
          <div className="flex gap-3 overflow-x-auto -mx-0 px-5 pb-2 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {tenant.reviews.map((review, i) => (
              <div key={`${review.author}-${i}`} className="shrink-0 w-[220px] bg-[#1E1B15] border border-[#2A2520] rounded-2xl p-4">
      ...
      {/* ── Mapa / Ubicación ──────────────────────────────────────── */}
      <section aria-labelledby="map-heading" className="px-5 pt-8 pb-2">
        ...
        <div className="bg-[#1E1B15] border border-[#2A2520] rounded-2xl overflow-hidden">
          {tenant.business.map_embed_url ? (
            ...
            <div className="h-36 bg-[#161410] flex items-center justify-center relative overflow-hidden">
      ...
      <footer className="px-5 mt-8 pb-2 border-t border-[#2A2520] pt-5 text-sm">
